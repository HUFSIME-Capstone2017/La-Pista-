...
def deftsp(air,depday,arrday,result_st,result_air,result_dep,alpha,beta,ST,lastcity,special_date):
    import datetime
    from fractions import Fraction
    ...
    arrday=datetime.date(int(arrday[0:4]),int(arrday[5:7]),int(arrday[8:10]))
        #exeption
    # for i in special_date:
    #     a= datetime.date(int(i[0:4]),int(i[5:7]),int(i[8:10]))
    #     if a < arrday and a>depday:
    #         answer=error
    ...
    scale=84*float(Fraction(beta,10*
        alpha))
    dfd = [0]
    ...
    for i in range(nbairports):
        difname = "dif_" + str(i)
        dif.append(difname)
    c.variables.add(names=dif, lb=[0.0] * len(dif), obj=dfd)
    for i in range(nbairports):
        thevars = []
        thecoefs = []
        for j in range(nbairports):
    ...
    for i in range(nbairports):
        if special_date[i] != 'None':
            thevars = [arr[i]]
            thecoefs = [1]
            spcity = datetime.date(int(special_date[i][0:4]),int(special_date[i][5:7]),int(special_date[i][8:10]))
            spcity = datetime.date.toordinal(spcity)
            c.linear_constraints.add([cplex.SparsePair(thevars,thecoefs)],                             
                                 senses = "L" ,
                                 rhs = [spcity+1] )   
